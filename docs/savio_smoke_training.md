...
After smoke passes, you can submit a **multi-epoch** job that still uses the same scratch data dir but **does not** use smoke mode (`BRIDGE_RNA_SMOKE=0`). See `scripts/savio_dev_train_savio2_1080ti.slurm`: default **1 GPU**, **`torchrun --nproc_per_node=1`**, **5 epochs**, fixed **20k / 4k** train/val, **batch 4**, **768 / 3072** / **2 layers** / **8 heads** (Minggang-style hyperparams on `train.py`; override env vars as needed; use `BRIDGE_RNA_USE_ALL_SAMPLES=1` for **80/20 on all samples**), **24 h** wall clock. The file comments describe how to switch back to **4× GPU** DDP if needed.
