...
After smoke passes, you can submit a **multi-epoch** job that still uses the same scratch data dir but **does not** use smoke mode (`BRIDGE_RNA_SMOKE=0`). See `scripts/savio_dev_train_savio2_1080ti.slurm`: default **1 GPU**, **`torchrun --nproc_per_node=1`**, **5 epochs**, fixed **10k / 2k** train/val (default; **20k / 4k** via env for full comparison), **batch 4**, **768 / 3072** / **2 layers** / **8 heads** (override env vars as needed; use `BRIDGE_RNA_USE_ALL_SAMPLES=1` for **80/20 on all samples**), **24 h** wall clock. The file comments describe how to switch back to **4× GPU** DDP if needed.
...
See **`docs/savio_wandb_sweeps.md`** — W&B sweeps via `sweeps/walt_sweep.yaml` (current: loss × LR × batch_size **grid**) and `scripts/savio_wandb_sweep_agent.slurm`.
